...
max_cnt = 0
slide = 0
repeat_slides = []                                           #список для ключей шифра, которые с большей вероятностью могут быть заданы
repeat_ind = 0

for i in range(1, len(slides)):                              #находим наиболее вероятные ключи шифра
...
for i in repeat_slides:                                    #проходим по всем найденным наиболее вероятным ключам и записываем расшифрованные сообщения в файл
    text = hack(i)
    f.write('key is ')
    f.write(str(i))
    f.write(' --- ')
